...
public class NotificationController {
    ...
    // ── SEND NOTIFICATION (Admin / Placement Team) ─────────────────────────
    @PostMapping("/send")
    @PreAuthorize("hasAnyRole('ADMIN','PLACEMENT_TEAM','DEPARTMENT_COORDINATOR')")
    public ResponseEntity<ApiResponse<Void>> send(
            @Valid @RequestBody NotificationRequest req,
            HttpServletRequest httpReq) {
        Long senderId = extractUserId(httpReq);
        notificationService.sendNotification(req, senderId);
        return ResponseEntity.ok(
            ApiResponse.success("Notification sent successfully", null));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> markRead(
            ...
            HttpServletRequest req) {
        notificationService.markAsRead(id, extractUserId(req));
    ...
    public ResponseEntity<ApiResponse<Void>> markAllRead(
            HttpServletRequest req) {
        notificationService.markAllAsRead(extractUserId(req));
        return ResponseEntity.ok(
